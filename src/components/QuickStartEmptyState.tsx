  ...
  return (
    <div className={className}>
      <div className="rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-50 to-purple-50 p-10 text-center">
        <Icon className="mx-auto mb-5 h-16 w-16 text-violet-400" />
        <h3 className="mb-2 text-2xl font-bold text-violet-900">{title}</h3>
        <p className="mx-auto mb-6 max-w-2xl text-violet-700">{description}</p>
        <div className="flex flex-wrap justify-center gap-3">
          {primaryAction.href ? (
            ...
            <Button onClick={primaryAction.onClick} className="gap-2 bg-gradient-to-r from-violet-600 to-purple-600 text-white hover:from-violet-700 hover:to-purple-700">
              {PrimaryIcon ? <PrimaryIcon className="h-4 w-4" /> : null}
              {primaryAction.label}
            </Button>
          )}
          ...
          {secondaryAction ? (
            secondaryAction.href ? (
              ...
              <Button onClick={secondaryAction.onClick} variant="outline" className="border-violet-600 text-violet-600 hover:bg-violet-50">
